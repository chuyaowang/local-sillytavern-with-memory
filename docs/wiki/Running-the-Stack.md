...
This assumes [Prerequisites](Prerequisites.md) are done: Docker Engine, the NVIDIA Container Toolkit, and (optionally) Tailscale all working.
...
The model files aren't in this repo (they're multi-gigabytes). Drop one in `models/`, then point `llama-cpp/models-preset.ini`'s `model =` line at its filename.
...
Edit that file and set a real `basicAuthUser.username`/`password`, and add the actual Tailscale IPs of whatever devices should be able to reach it to the `whitelist` array (`tailscale status` shows you those IPs).

## 4. Get the embedding model

mem0 uses this to turn memories into vectors for Qdrant. Download it straight into `models/`:
...
- SillyTavern: `http://localhost:8000` locally, or `http://<tailscale-ip-of-host>:8000` from a whitelisted device over Tailscale.
