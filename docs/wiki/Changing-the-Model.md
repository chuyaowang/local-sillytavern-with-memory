...
1. **Download a GGUF.** Grab one from Hugging Face or other sources. Place it in `models/`. The actual VRAM used is usually smaller than the GGUF file's size — the token embedding table typically stays on CPU.
...
Q8 fits, with a real speed tradeoff when the embedder also needs VRAM — see [docs/LLAMA_CPP_BENCHMARK.md](https://github.com/chuyaowang/local-sillytavern-with-memory/blob/main/docs/LLAMA_CPP_BENCHMARK.md) for the full story. Q4 is the safer everyday default.
